...
CREATE DATABASE StudentCareer_db;
USE StudentCareer_db;
...
INSERT INTO StudentCareer VALUES (88,'Nisha_88',24,'Female','BE CSE','Finance',9.2,1,0.0,0,'GenderTraveling','Finance','Marketing','Marketing Manager','Hyderabad',900000,'Research','Remote','Beginner','Intermediate','Average','Good',3,'Yes','No','Excellent','Placed');
...
# Q10. Write an SQL query to display students interested in: Artificial Intelligence, Data Science , HR Analytics 
SELECT *
FROM studentcareer
...
select Experience_Years,
	case
		when Experience_Years < 0.5 then 'Fresher'
        when Experience_Years <= 1.0 then 'Junior'
        else 'Experienced'
	end as Category
from studentcareer;
...
select Expected_Salary,
	case
        ...
        when Expected_Salary <= 1100000 then 'Medium'
        else 'High'
	end as Category
from studentcareer;
...
select Student_Name, Department, CGPA,
	row_number() over (
