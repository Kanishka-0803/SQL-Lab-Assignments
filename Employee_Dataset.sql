...
-- 🔹 PART D – GROUP BY & HAVING (Single Table)
...
SELECT mother_tongue, avg(salary)
FROM employee
GROUP BY mother_tongue
...
-- Q50 Display employees who joined in the same year as their experience value
SELECT *
FROM employee
WHERE year(doj) = experience;


-- ⭐ Bonus / Challenge Questions

-- B1 Categorize Employees using CASE
...
-- B2 Calculate current age using DOB
SELECT emp_name, dob, timestampdiff(year, dob, curdate()) as Current_Age
FROM employee;

-- B3 Calculate experience from DOJ
SELECT emp_name, doj, timestampdiff(year, doj, curdate()) as Calculated_Experience
FROM employee;

-- B4 Employees above department average salary
SELECT *
FROM employee e1
WHERE salary >
(
...
-- B5 Identify employees eligible for promotion
SELECT *
FROM employee
WHERE experience >= 5
AND salary >= 50000;